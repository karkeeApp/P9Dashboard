...
import { AdsGeneralDescription } from '@/components/ads';

const { TabPane } = Tabs;

const AdsDetailsView: FC<RouteComponentProps<{ id: string }>> = ({
  history,
  match,
}) => {
  ...
  const fetchAds = useCallback(async () => {
    try {
      ...
      const { data } = await API.get<ResponseData<Ads>>('/ads/view', {
        params: {
          id,
        },
      });
        ...
        <div className='container'>
          ...
          >
            <Flex
              ...
              alignItems='baseline'
            >
              <h2 className='mr-2'>{ads.title}</h2>
...
export default memo(AdsDetailsView);
/* eslint-enable */
