...
import { User, AdminSetting } from '@/types/user';
...
} from '@/components/user/UserActionButton';
...
interface ListState {
  ...
  total: number;
  users: User[];
  selectedRole: AdminSetting['id'] | 'all';
  selectedType: AdminSetting['id'] | 'all';
  selectedStatus: AdminSetting['id'] | 'all';
}

const UserListView: FC<RouteComponentProps> = ({ history }) => {
  ...
  const handleRemove: UserActionButtonProps['onRemove'] = () => {
    void fetchUsers();
  };
      ...
      render: (fullname: User['fullname'], user) => (
        <div className='d-flex'>
          <AvatarStatus
            ...
            onNameClick={() =>
              history.push({
                pathname: `${APP_PREFIX_PATH}/users/${user.user_id}`,
              })
            }
          />
        </div>
    ...
    },
    {
      ...
      render: (user: User) => (
        <div className='text-right'>
          <UserActionButton user={user} onRemove={handleRemove} />
        </div>
      ),
    },
  ];
        ...
        <Flex className='mb-1' mobileFlex={false}>
          <div className='mb-3'>
            <Button
              ...
              onClick={() =>
                history.push({
                  pathname: `${APP_PREFIX_PATH}/users/add`,
                })
              }
            >
              Add User
